...
// Karyawan tidak boleh edit transaksi yang sudah terjadi (rawan fraud)
$role = $_SESSION['role'];
if ($role !== 'owner') {
    echo "<script>
            alert('Akses Ditolak! Hanya Owner yang boleh mengedit transaksi.'); 
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($bahan_ids)) {
        ...
        try {
            // 1. ROLLBACK STOK LAMA (Kurangi stok karena pembelian lama dihapus sementara)
            $old_items = $conn->query("SELECT id_bahan, jumlah FROM detail_pembelian WHERE id_beli = '$id_beli'");
            while ($old = $old_items->fetch_assoc()) {
                // Kurangi stok (karena transaksi ini dianggap batal dulu sebelum diupdate)
            ...
            // 3. INSERT BARU & TAMBAH STOK BARU
            $total_beli = 0;
            ...
            for ($i = 0; $i < count($bahan_ids); $i++) {
                ...
                // Update Stok (Tambah lagi sesuai revisi)
...
if(!$header) { header("Location: index.php"); exit(); } // Cek jika ID tidak valid
    ...
    <div class="main-content">
        <div class="topbar shadow-sm">
            <div class="d-flex align-items-center gap-3">
                ...
                <div class="page-title"><h4 class="m-0">Edit Transaksi</h4></div>
            </div>
            ...
            <div class="card shadow-sm border-0 rounded-4">
                ...
                <div class="card-body">
                    ...
                    <form method="POST">
                                ...
                                <tbody id="pembelianItems">
                                    <?php foreach ($details as $d): ?>
                                        <tr class="item-row">
                                            ...
                                            <td>
                                                <input type="number" name="harga_satuan[]" class="form-control input-harga text-end" value="<?php echo $d['harga_satuan']; ?>" onkeyup="hitungSub(this)" required>
                                            </td>
                                            <td>
                                                <input type="number" name="jumlah[]" class="form-control input-qty text-center" value="<?php echo $d['jumlah']; ?>" min="1" onkeyup="hitungSub(this)" onchange="hitungSub(this)" required>
                                            </td>
                                            <td>
                                                <input type="text" class="form-control input-sub text-end bg-light" value="<?php echo number_format($d['sub_total'], 0, ',', '.'); ?>" readonly>
                                            </td>
                        ...
                        <button type="button" class="btn btn-outline-success btn-sm mb-3" onclick="addRow()"><i class="bi bi-plus-lg"></i> Tambah Bahan</button>

                        <div class="row justify-content-end">
                            <div class="col-md-4">
                                <div class="input-group">
                                    <span class="input-group-text fw-bold">Total</span>
                                    <input type="text" id="grandTotal" class="form-control fw-bold text-end bg-light" value="Rp <?php echo number_format($header['total_beli'], 0, ',', '.'); ?>" readonly>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="d-flex justify-content-end gap-2 mt-4">
                            ...
                            <button type="submit" class="btn btn-brown px-4">Simpan Perubahan</button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    <script>
        ...
        function addRow() {
            ...
            <tr class="item-row">
                ...
                <td><input type="number" name="harga_satuan[]" class="form-control input-harga text-end" placeholder="0" onkeyup="hitungSub(this)" required></td>
                <td><input type="number" name="jumlah[]" class="form-control input-qty text-center" value="1" min="1" onkeyup="hitungSub(this)" onchange="hitungSub(this)" required></td>
                <td><input type="text" class="form-control input-sub text-end bg-light" readonly></td>
                <td><button type="button" class="btn btn-danger btn-sm" onclick="delRow(this)"><i class="bi bi-trash"></i></button></td>
            </tr>`;
